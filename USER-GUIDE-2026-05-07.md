...
- **Margin Multiplier column** added to the Fees table on the Setup tab — exposes the previously-hidden `marginMultiple` field. Set Mode = `marginLinked` and Margin Mult = `0.35` to model the UK 35%-of-margin convention. Rate column auto-disables for margin-linked rows.
- **Reconcile tab** — new top-level tab. Load period-level results from a 3rd-party system (branded **PortF Data**) and compare against the calculator's output line-by-line: status (tied / within tolerance / break), summary KPIs, breaks-only filter, breaks CSV export. PortF data persists per-instrument in localStorage.
- **DIU Source selector** — the DIU Export tab now has a dropdown to switch between *Our calculated data (engine)* and *PortF Data (loaded on Reconcile tab)*. Both sources render the same JE pair shape and download with the same CSV columns.
- **Investran GL chart mapping** — every JE row is now mapped to a real Investran GL account / transaction type per `GL Accounts and tran types.xlsx`. A **GL Coverage panel** above the JE grid flags accounts/transaction types that don't yet exist in the chart and surfaces concrete recommendations for new lines (e.g., 470000 Impairment / ECL Expense, 360000 Cash Flow Hedge Reserve OCI). Full inventory in `gl-account-gaps.md`.
- **Hedge accounting UI** — Setup tab Hedge Accounting card with editable controls for hedge type (CFH / FVH), notional, leg rates, effectiveness ratio, Fair Value Schedule (date+MTM rows), Settlement Dates list. Worked example: **Libra 3** is fully configured.
- **Libra 2 reverted** to the requirements XLSX exactly — two drawdowns (£15M actual + £10M forecast). The hedge example moved into Libra 3 so Libra 2 remains a faithful match of the source data.
...
21. [Common scenarios — how-to](#21-common-scenarios)
...
- **Our calculated data (engine)** — default. The DIU pairs come from the daily schedule built by `buildSchedule()` summed over the active accrual window.
- **PortF Data (loaded on Reconcile tab)** — the DIU pairs come from the PortF reference dataset loaded via the Reconcile tab. Useful when you want to post the PortF numbers downstream (e.g., during a parallel-run period) while you investigate the variances.

Switching the source re-renders the JE grid and updates the **GL Coverage panel** (below). The CSV download honours the selected source — file names are `diu-export.csv` (engine) or `diu-export-portf.csv` (PortF).
...
- **Clean** — the transaction type maps directly to an existing Investran account + transtype (green status).
- **Gap** — no clean Investran transtype exists; the engine uses a placeholder account and surfaces the gap with a `GAP` chip on the JE row, plus a row in the Coverage panel with the recommended new account/transtype name.

For the canonical inventory of all gaps across the 13 seed instruments and the priority list of new accounts your Investran administrator should create, see `gl-account-gaps.md`. Section [§19](#19-investran-gl-chart-coverage--gaps) below summarises.
...
- Non-use fee pair *(GAP — see §19)*
- Per-fee IFRS 15 pairs (Arrangement, Commitment, Guarantee, NWF Commitment, Management Fee, Dividend) *(GAP — see §19)*
- IFRS 9 EIR fee accretion pair (141000 / 421000)
- Default interest + default fee pairs *(GAP — see §19)*
- ECL provisioning pair *(GAP — see §19)*
- FX revaluation pair (450000 unrealised / 440000 realised)
- Modification gain/loss pair *(GAP — see §19)*
- Hedge accounting pairs *(GAP — see §19)*
- **Cash-leg JEs** — drawdowns (141000 / 111000), repayments (111000 / 141000), and cash-settlement of every accrued receivable on payment dates
...
| **"Upload .json"** | File picker — pick a JSON file matching the template shape. |
...
- **Status chip** — green (no breaks), amber (≤3 breaks), red (>3 breaks). Includes tie-rate %.
- **5 KPI cards** — Periods compared / Lines compared / Tied (with %) / Within tolerance / Breaks (with total |Δ|).
- **Per-line diff grid** — Period End | Metric | PortF | Ours | Δ | Δ% | Status. Rows are colour-coded by status; break rows are highlighted yellow.
- **"Show breaks only" toggle** — collapses to just the failing lines.
- **Breaks CSV** — downloads only the break lines for offline investigation.
- **Clear** — drops the PortF dataset for this instrument from localStorage.
...
- Per metric: classify as **tied** (within absolute tolerance), **within** (absolute tol breached but Δ% ≤ break threshold), **break** (>break threshold).

The reconciliation engine is `reconcileAgainstReference(rows, refData)` — pure function, easy to unit-test.
...
|---|---|---|---|
| 1 | IFRS 15 fee income | Libra 2/3, Volt, Volt Multi-Loan, XYZ Buyout, ABCDEF | **492100 Arrangement Fee Income · 492200 Commitment Fee Income · 492300 Guarantee Fee Income · 492400 Management Fee Income · 492500 Dividend Income (Equity)** — or sub-transtypes under existing 492000 |
| 2 | IFRS 15 fee receivables | Same set | Add transtypes under **113000**: `Fee receivable - Arrangement / Commitment / Guarantee / Management / Dividend (Equity)` plus matching `Fee received - …` for cash settlements |
| 3 | IFRS 9 ECL impairment expense | Libra 2/3, Volt | **470000 Impairment / ECL Expense (IFRS 9 §5.5)** — strongly recommended |
| 4 | IFRS 9 ECL loan loss allowance | Libra 2/3, Volt | **145000 Loan Loss Allowance** (contra under 141000) — strongly recommended |
| 5 | Hedge accounting — derivative asset | Libra 3 | **146000 Derivative Assets / Liabilities** |
| 6 | Hedge accounting — Cash Flow Hedge Reserve OCI | Libra 3 | **360000 Cash Flow Hedge Reserve (OCI)** — strongly recommended (separate equity component per IFRS 9 §6.5.11) |
| 7 | Hedge accounting — ineffectiveness / FV hedge P&L | Libra 3 | **451000 Hedge Ineffectiveness P&L · 452000 Fair Value Hedge P&L** |
...
| 9 | IFRS 9 §5.4.3 modification gain/loss | None in seed (data-driven) | **442000 Modification Gain/Loss** (recommended for cleaner reporting) |
...
|---|:-:|---|
| Alliance, Copperleaf, Orion×2, Meridian, Suffolk Solar | 0 | All vanilla — 100% clean mapping |
...
After Investran creates these, update `INVESTRAN_GL` in `income-calculator.html` — change each gap entry's `account` and `transType` to point at the new codes and remove the `gap: true` flag. The DIU GL Coverage panel will turn green.
